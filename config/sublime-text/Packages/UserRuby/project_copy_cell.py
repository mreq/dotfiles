            ...
            if i != -1:
                ...
                for folder_name in self.window.folders():
                    if folder_name in file_name:
                        ...
                        names = (
                            bytes.decode(b).splitlines() + bytes.decode(c).splitlines()
                        )
                        ...
                        for name in names:
                            if "cell" in name:
                                ...
                                target_path = (
                                    target_dir + "/" + name.replace(source_name, target)
                                )
                                target_dirpath = os.path.dirname(target_path)
                                subprocess.call(
                                    ["mkdir", "-p", target_dirpath], cwd=folder_name
                                )
                                subprocess.call(
                                    ["cp", name, target_path], cwd=folder_name
                                )
