...
class GitShowFromDiff(sublime_plugin.WindowCommand):
    def run(self):
        ...
        if row < 0:
            row = 0
        col = 0
        target_region = sublime.Region(view.text_point(row, col), view.text_point(row+20, col))
        first_line = view.substr(view.line(view.text_point(0, 0)))

        before_commit = re.search(r'commit\s+(.+)', first_line).groups()[0]
        target_commit = subprocess.check_output("git log --pretty=oneline | grep -A1 "+before_commit+" | tail -n1 | awk '{ print $1 }'", shell=True).strip().decode('utf-8')
